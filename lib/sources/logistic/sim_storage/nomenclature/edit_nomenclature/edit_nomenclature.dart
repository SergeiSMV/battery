...
import 'editnomenclature_category.dart';
import 'editnomenclature_producer.dart';
import 'editnomenclature_unit.dart';



String funcDescription = 'Редактирования номенклатуры склада СиМ';


class EditNomenclarure{
  final BuildContext nomenclatureContext;
  final List currentState;
  final int index;
  const EditNomenclarure({required this.nomenclatureContext, required this.currentState, required this.index});
    ...
    return showModalBottomSheet(
      ...
      builder: (context){
        return MultiBlocProvider(
          providers: [
            BlocProvider<NomenclatureEditBloc>(create: (context) => NomenclatureEditBloc()..add(NomenclatureEditChange(data: currentState[index]))),
            BlocProvider<IsTappedBloc>(create: (context) => IsTappedBloc()),
          ], 
          child: Builder(builder: (context){

            String category = context.watch<NomenclatureEditBloc>().state['category'].toString();
            String name = context.watch<NomenclatureEditBloc>().state['name'].toString();
            String producer = context.watch<NomenclatureEditBloc>().state['producer'].toString();
            // String palletWeight = context.watch<NomenclatureEditBloc>().state['pallet_weight'].toString();
            // String palletSize = context.watch<NomenclatureEditBloc>().state['pallet_size'].toString();
            // String boxWeight = context.watch<NomenclatureEditBloc>().state['box_weight'].toString();
            String boxSize = context.watch<NomenclatureEditBloc>().state['box_size'].toString();
            // String palletQuant = context.watch<NomenclatureEditBloc>().state['pallet_quant'].toString();
            String boxQuant = context.watch<NomenclatureEditBloc>().state['box_quantity'].toString();
            String unit = context.watch<NomenclatureEditBloc>().state['unit'].toString();

            bool isTapped = context.watch<IsTappedBloc>().state;
            Map editState = context.watch<NomenclatureEditBloc>().state;

            Function eq = const ListEquality().equals;
            bool compire = eq([currentState[index]], [editState]);
                      ...
                      child: Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Expanded(child: Text('редактирование номенклатуры', style: TextStyle(color: firmColor, fontSize: 16))),
                          ],
                        ),
                      )
                    ),
                  ...
                  Flexible(
                      child: ListView(
                        ...
                        children: [
                          ...
                          Row(
                            children: [
                              Expanded(child: _enterValue('категория:', category, TextInputType.text, (value) { 
                                ...
                                context.read<NomenclatureEditBloc>().add(NomenclatureEditChange(data: Map.from(editState)));
                              },),),
                              IconButton(onPressed: (){
                                EditNomenclatureCategory(parrentContext: context, currentState: editState).getProducers();
                          ...
                          _itemElement('поставщик:', producer, 'укажите поставщика', () => EditNomenclatureProducer(currentState: editState, parrentContext: context).getProducers()),
                          _itemElement('ед. измерения:', unit, 'укажите ед. измерения', () => EditNomenclatureUnit(currentState: editState, parrentContext: context).getUnits()),
                  ...
                  isTapped == false ?
                    Container(
                      ...
                      child: TextButton(onPressed: () async {
                        ...
                        await Connection(data: editState, route: simUpdateNomenclature).connect().then((value){
                          value == 'done' ? 
                          {
                            currentState[index] = editState,
              ...
              child: Padding(
                ...
                child: TextField(
                  ...
                  decoration: InputDecoration(
                    ...
                    hintStyle: TextStyle(fontSize: 12, color: firmColor, fontFamily: 'Montserrat'),
                    hintText: hint,
                  ),
