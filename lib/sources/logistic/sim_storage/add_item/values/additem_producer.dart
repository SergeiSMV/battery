...
class AddItemProducer{
  final BuildContext parrentContext;
  final Map currentState;
  final List controllers;
  const AddItemProducer({required this.parrentContext, required this.currentState, required this.controllers});

  getProducer() {
    currentState['category'].isEmpty || currentState['name'].isEmpty ?
    {
      ProgressHUD.of(parrentContext)?.dismiss(),
      systemMessage(parrentContext, 'заполните поля выше', 'lib/images/lottie/close.json')
    } :
    Connection(data: {'category': currentState['category'], 'name': currentState['name']}, route: simNmProducers).connect().then((value) => _checkResponce(value));
  }
  ...
  _checkResponce(dynamic responce){
    ...
    if(responce is String){
      ...
      return _setProducer(responce);
    }
  }

  _setProducer(List producers){
        ...
        return Container(
          ...
          child: Column(
            ...
            children: [
              ...
              Text('выбирите поставщика', style: TextStyle(color: firmColor, fontSize: 12), textAlign: TextAlign.center,),
                    ...
                    itemBuilder: (contex, index){
                      return Padding(
                        ...
                        child: ListTile(
                          ...
                          title: Text(producers[index], textAlign: TextAlign.center, style: TextStyle(color: firmColor, fontSize: 16)),
                          onTap: (){
                            currentState['producer'] == producers[index].toString() ? null : {
                              currentState['producer'] = producers[index].toString(),
                              currentState['color'] = '',
                              currentState['unit'] = '',
